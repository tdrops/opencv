...
blur = cv2.GaussianBlur(src=img_gray, ksize=(3,3), sigmaX=0)
circles = cv2.HoughCircles(image=blur, method=cv2.HOUGH_GRADIENT, dp=1.5, minDist=30, circles=None, param1=200)
if circles is not None:
    circles = np.uint16(np.around(circles))
    for i in circles[0,:]:
        cv2.circle(img=img, center=(i[0],i[1]), radius=i[2], color=(0,255,0), thickness=1)
        cv2.circle(img=img, center=(i[0],i[1]),radius=2, color=(0,0,255), thickness=-1)
