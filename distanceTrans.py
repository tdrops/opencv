...
_,th = cv2.threshold(src=img, thresh=127, maxval=255, type=cv2.THRESH_BINARY_INV)
dst = cv2.distanceTransform(src=th, distanceType=cv2.DIST_L2, maskSize=5)
dst = ((dst*255)/(dst.max()-dst.min())).astype(np.uint8)
skeleton = cv2.adaptiveThreshold(src=dst, maxValue=255, adaptiveMethod=cv2.ADAPTIVE_THRESH_GAUSSIAN_C, thresholdType=cv2.THRESH_BINARY, blockSize=7, C=-3)
